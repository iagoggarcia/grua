...
DatosCamara calcularCamara(const Grua& grua, int modoCamara) {
    ...
    // Convertimos la dirección de la grúa a radianes para poder obtener el vector frontal en el plano XZ.
    float rad = glm::radians(grua.direccion);
    datos.frente = glm::vec3(sin(rad), 0.0f, cos(rad));

    if (modoCamara == 1) {
        // Cámara cercana y elevada, colocada detrás de la grúa y orientada hacia la zona delantera.
        ...
        // Cámara más lejana, también situada detrás de la grúa,
        // para obtener una vista más general de la escena.
        ...
        // Cámara fija en una posición global de la escena.
        datos.camPos = glm::vec3(55.0f, 23.0f, 10.0f);
        datos.camTarget = glm::vec3(0.0f, 0.0f, 0.0f);
    }

    // Construimos la matriz de vista a partir de la posición de la cámara,
    // el punto al que mira y el vector vertical del mundo.
...
{
    // Enviamos al shader la matriz de vista de la cámara.
    glUniformMatrix4fv(glGetUniformLocation(shaderProgram, "view"), 1, GL_FALSE, glm::value_ptr(camara.view));

    // Enviamos también la matriz de proyección.
    glUniformMatrix4fv( glGetUniformLocation(shaderProgram, "projection"), 1, GL_FALSE, glm::value_ptr(projection));

    // Pasamos la posición de la cámara para los cálculos de iluminación.

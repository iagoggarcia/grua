...
static void dibujarPieza(const Pieza& pieza, const glm::mat4& modeloPadre, GLuint shaderProgram, float r, float g, float b) {
    ...
    // Indicamos al shader que, para esta pieza, use un color uniforme en vez del color asociado a los vértices.
    ...
    glDrawArrays(GL_TRIANGLES, 0, 36); // 36 vértices forman los 12 triángulos del cubo
...
static void dibujarBrazo(const Pieza& brazo, const glm::mat4& modeloArticulacion, GLuint shaderProgram, float r, float g, float b) {
    ...
    // Desplazamos el cubo medio brazo hacia delante para que el giro y la unión

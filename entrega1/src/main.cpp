...
void processInput(GLFWwindow* window, Grua& grua, int& modoCamara, float deltaTime);
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        processInput(window, grua, modoCamara, deltaTime);

        float rad = glm::radians(grua.direccion);
        grua.posicion.x += sin(rad) * grua.velocidad * deltaTime;
        grua.posicion.z += cos(rad) * grua.velocidad * deltaTime;

        // Rozamiento simple
        grua.velocidad *= (1.0f - 1.5f * deltaTime);
        if (grua.velocidad < 0.0f) grua.velocidad = 0.0f;
